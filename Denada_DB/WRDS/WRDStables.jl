...
function driftweights!(CRSPdf)
    ...
    CRSPdf = groupcumret!(CRSPdf, [:permno, :ffyear], :retx)
    # Lag :cumretx (the cumulated return ex dividend in the previous period)
    CRSPdf = grouplag!(CRSPdf, :permno, :cumretx, 1)

    # lag market cap
    CRSPdf = grouplag!(CRSPdf, :permno, :me, 1)
    CRSPdf[[:date, :me, :lagme_1]]

    # if first permno then use me/(1+retx) to replace the missing value
    CRSPdf = setfirstlme!(CRSPdf, :permno, :lagme_1)
