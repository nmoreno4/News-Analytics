...
daterange = ["01/01/2000", "12/31/2017"]
...
if fundfq == "q"
    CSvariables = "gvkey, datadate, cusip, conm, atq, ceqq, ibq, ltq, revtq, saleq, seqq, txditcq, xintq, pstkq"
    CSdatatable = "comp.fundq"
elseif fundfq=="a"
    CSvariables = "gvkey, datadate, cusip, conm, at, ceq, lt, seq, txditc, pstkrv, pstkl, pstk"
    CSdatatable = "comp.funda"
end
if exchfq ==  "m"
    #Spread is only available at monthly frequency.
    CRSPvariables = "a.permno, a.permco, a.date, b.shrcd, b.exchcd, a.ret, a.retx, a.shrout, a.prc, a.vol, a.spread"
    CRSPdatatable = ["crsp.msf", "crsp.msenames"]
elseif exchfq=="d"
    CRSPvariables = "a.permno, a.permco, a.date, b.shrcd, b.exchcd, a.ret, a.retx, a.shrout, a.prc, a.vol"
    CRSPdatatable = ["crsp.dsf", "crsp.dsenames"]
end

# CSdf, CRSPdf, delistdf, CCMdf = gatherWRDSdata()

CSdf = CSdownload(daterange, CSvariables, CSdatatable)
...
#####===CRSP====###
CRSPdf = CRSPdownload(daterange, CRSPvariables, CRSPdatatable)
...
ccm1 = ccm1[[:gvkey, :permno, :datadate, :yearend, :jdate, :be]]
...
june = ccm_jun[[:permno, :date, :jdate, :rankbm, :ranksize, :posbm, :nonmissport, :ptf_10by10_size_value, :ptf_2by3_size_value, :ptf_5by5_size_value]]
...
CRSPvariables = "a.permno, a.date,a.ret"
CRSPdatatable = ["crsp.dsf", "crsp.dsenames"]
daterange = ["01/01/2003", "12/31/2017"]
CRSPdf = CRSPdownload(daterange, CRSPvariables, CRSPdatatable)
...
CRSPdf = CRSPdf[[:year, :month, :permno, :retadj, :date]]
names!(CRSPdf, [:year, :month, :permno, :dailyretadj, :dailydate])
Result = join(CRSPdf, ccm4, kind=:left, on=[:year, :month, :permno])
Result[[:date, :dailyretadj, :rankbm, :permno]]
...
using Mongo
client = MongoClient()
CRSPConnect = MongoCollection(client, "NewsDB", "DailyCRSP2")
cc=0
